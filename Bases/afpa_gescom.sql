...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : jeu. 22 oct. 2020 à 14:47
...
-- Procédures
--
DROP PROCEDURE IF EXISTS `ajoutFournisseur`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ajoutFournisseur` (IN `p_nom` VARCHAR(50), IN `p_countries_id` CHAR(2), IN `p_address` VARCHAR(150), IN `p_zipcode` VARCHAR(5), IN `p_contact` VARCHAR(100), IN `p_phone` VARCHAR(10), IN `p_mail` VARCHAR(75))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CA_Supplier` (`p_sup_id` INT(10), `p_order_date` YEAR)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Facture` (IN `p_num_commande` INT(10))  BEGIN
    DECLARE tva float;
    DECLARE total float;
    SET tva = 20;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Lst_Rush_Orders` ()  BEGIN
    SELECT *
    FROM orders
...
(8, 27, '249.99', 'barb002', NULL, 7, 5, 'Noir', 'Athos', 'Cu\'rabitur pellentesque posuere luctus. Sed et risus vel quam pharetra pretium non quis odio. Praesent varius risus vel orci ultrices tincidunt.', 0, 4, '2016-06-14 00:00:00', NULL, 'jpg'),
...
(6, 'Bleu', 'Amiens', 'FR', 'Avenue du lys', '75000', 'JP', '0699009900', 'bleu@abc.def');
...
CREATE TABLE IF NOT EXISTS `v_catalogue` (
`cat_name` varchar(50)
...
--
-- Doublure de structure pour la vue `v_ventes_barb`
-- (Voir ci-dessous la vue réelle)
--
DROP VIEW IF EXISTS `v_ventes_barb`;
CREATE TABLE IF NOT EXISTS `v_ventes_barb` (
...
DROP VIEW IF EXISTS `v_catalogue`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_catalogue`  AS  select `products`.`pro_id` AS `pro_id`,`products`.`pro_ref` AS `pro_ref`,`products`.`pro_name` AS `pro_name`,`products`.`pro_cat_id` AS `pro_cat_id`,`categories`.`cat_name` AS `cat_name` from (`products` join `categories` on((`products`.`pro_cat_id` = `categories`.`cat_id`))) ;
...
DROP VIEW IF EXISTS `v_details`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_details`  AS  select `orders_details`.`ode_pro_id` AS `ode_pro_id`,sum(`orders_details`.`ode_quantity`) AS `QteTot`,truncate(sum(((`orders_details`.`ode_unit_price` * `orders_details`.`ode_quantity`) * ((100 - `orders_details`.`ode_discount`) / 100))),2) AS `PrixTot` from `orders_details` group by `orders_details`.`ode_pro_id` ;

-- --------------------------------------------------------

--
-- Structure de la vue `v_ventes_barb`
--
DROP TABLE IF EXISTS `v_ventes_barb`;

DROP VIEW IF EXISTS `v_ventes_barb`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_ventes_barb`  AS  select `orders_details`.`ode_id` AS `ode_id`,`orders_details`.`ode_unit_price` AS `ode_unit_price`,`orders_details`.`ode_discount` AS `ode_discount`,`orders_details`.`ode_quantity` AS `ode_quantity`,`orders_details`.`ode_ord_id` AS `ode_ord_id`,`orders_details`.`ode_pro_id` AS `ode_pro_id` from `orders_details` where `orders_details`.`ode_pro_id` in (select `products`.`pro_id` from `products` where (`products`.`pro_ref` = 'barb001')) ;
...
DROP VIEW IF EXISTS `v_ventes_zoom`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_ventes_zoom`  AS  select `orders_details`.`ode_id` AS `ode_id`,`orders_details`.`ode_unit_price` AS `ode_unit_price`,`orders_details`.`ode_discount` AS `ode_discount`,`orders_details`.`ode_quantity` AS `ode_quantity`,`orders_details`.`ode_ord_id` AS `ode_ord_id`,`orders_details`.`ode_pro_id` AS `ode_pro_id` from `orders_details` where `orders_details`.`ode_pro_id` in (select `products`.`pro_id` from `products` where (`products`.`pro_ref` = 'ZOOM')) ;
